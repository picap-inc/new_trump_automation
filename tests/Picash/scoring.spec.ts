...
test.describe("Validación del módulo Picash", () => {
  test("Ingresar a Picash", async ({ page }) => {
    ...
    await test.step("Ingresar a Scoring", async () => {
      const sScoring = page.getByRole('link', { name: 'Scoring' });
      await expect(sScoring).toBeVisible({ timeout: 7000 });
      await sScoring.click();
      await expect(page).toHaveURL("https://admin.picap.io/picash/scoring", {
        timeout: 10000,
      });
      await capturarPaso(page, "05_scoring_picash", "picash");
    });
 });
});
*/