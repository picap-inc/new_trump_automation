...
test.describe("Validación del módulo Picash", () => {
  test("Ingresar a Picash", async ({ page }) => {
    ...
    await test.step("Ingresar a Recargas", async () => {
      // Click en 'Recargas'
      const recargas = page.getByRole('link', { name: 'Recargas' });
      await expect(recargas).toBeVisible({ timeout: 7000 });
      await recargas.click();

      // Validar URL
      await expect(page).toHaveURL("https://admin.picap.io/picash/recharges", {
        timeout: 10000,
      });

      // Captura de pantalla
      await capturarPaso(page, "05_recargas_picash", "picash");
    });
  });
});
