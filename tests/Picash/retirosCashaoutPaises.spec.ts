...
test.describe("Validación del módulo Picash", () => {
  test("Ingresar a Picash", async ({ page }) => {
    ...
    await test.step("Ingresar a Cashout Países desde Retiros", async () => {
      // Click en 'Retiros'
      const retiros = page.locator('a').filter({ hasText: 'Retiros' });
      await expect(retiros).toBeVisible({ timeout: 7000 });
      await retiros.click();

      // Click en 'Cashout Países'
      const cashoutPaises = page.getByRole('link', { name: 'Cashout Países' });
      await expect(cashoutPaises).toBeVisible({ timeout: 7000 });
      await cashoutPaises.click();

      // Validar URL
      await expect(page).toHaveURL("https://admin.picap.io/picash/withdrawals_cashout_countries", {
        timeout: 10000,
      });

      // Captura de pantalla
      await capturarPaso(page, "05_cashout_paises", "picash");
    });
  });
});
*/
